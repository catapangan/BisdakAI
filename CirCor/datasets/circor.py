...
class CirCor:
	...
	def quantize_audio(data, num_bits=8, compand=False, mu=255):
		...
		if compand:
			...
			data_ex = CirCor.expand(data_ex)
			q_data = np.round((data_ex - (-1.0)) / step_size)
			q_data = np.clip(q_data, 0, max_val)
		return np.uint8(q_data)

	def __gen_datasets(self, exp_len=4000, row_len=4000, overlap_ratio=0):
						...
						for n_r in range(num_rows):
							...
							if not self.save_unquantized:
								data_in[data_idx, :, n_r] = \
									CirCor.quantize_audio(audio_chunk,
...
def CirCor_get_datasets(data, load_train=True, load_test=True, num_classes=8):
	"""
	...
	if load_train:
		train_dataset = CirCor(root=data_dir, classes=classes, d_type='train',
	...
	if load_test:
		test_dataset = CirCor(root=data_dir, classes=classes, d_type='test',
							transform=transform, t_type='keyword',
							quantization_scheme=quantization_scheme,
							augmentation=augmentation)
